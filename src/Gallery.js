...
class GalleryPokemonRow extends React.Component {
	...
	render() {
		...
		return (
			<DetailWrapper pokemonNumber={pokemonId} imageUrl={imageUrl} modalManage = {this.props.modalManage} showModal = {this.props.showModal} pokemonCount={this.props.pokemonCount}/>
		);
	}
}

class GalleryPokemonTable extends React.Component {
	...
	render() {
		...
		rPokemonList.some((item) => {
			...
			if (count === this.state.pokemonCount) {
				rows.push(
					<GalleryPokemonRow className={styles.row}
						Pokemon={item}
	...
	render() {
		return (
			<div>
				...
				/>
				<GalleryPokemonTable  className={styles.GalleryPokemmonTable}
